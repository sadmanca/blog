...
# Hardcoded companyImages variable
company_images = {
...
# Directory to save images (relative to the current working directory)
output_dir = "assets/img/data/uoft-pey-coop-job-2024/"

# Update company_images with local paths
company_urls = {}
for company_name in company_images.keys():
    local_path = f"assets/img/data/uoft-pey-coop-job-2024/{company_name.replace(' ', '_').replace(',', '').replace('.', '').replace('(', '').replace(')', '').replace('&', 'and')}.png"
    company_name = "\"" + company_name + "\""
    company_urls[company_name] = "\"" + local_path + "\","

# Save updated company_images to a .txt file
with open("updated_company_images.txt", "w") as file:
    for company_name, local_path in company_urls.items():
        file.write(f"{company_name}: {local_path}\n")